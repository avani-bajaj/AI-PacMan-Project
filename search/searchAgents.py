...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextX, nextY = int(x + dx), int(y + dy)
            if not self.walls[nextX][nextY]:
                # Print (“reached if condition -  self.wall’)
...
def cornersHeuristic(state, problem):
    ...
    ## current position 
    position = state[0]
    ## check corners and if they are visited or not 
    Corners = state[1]
    ...
    Corners_Left = []
    for idx in Corners:
        Corners_Left.append(corners[idx])
    distanceList = []
    # In the maze - get distance of all corners from current position
    for corner in Corners_Left:
        # For efficiency, pacman may walk through an old state
        action = position + corner
        if action in problem.heuristicInfo.keys():
            distance = problem.heuristicInfo[action]
        else:
            distance = mazeDistance(position, corner, problem.gameState)
            problem.heuristicInfo[action] = distance
...
def foodHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        return 0
    # Distance checks the distance between the current position and all the food
    distance = []
    flag = 0
    for item in FoodList:
        ...
        flag = flag +1 
        if flag == 5 and problem.heuristicInfo['wallCount'] > 20:
            break
    return max(distance)
    #return 0
