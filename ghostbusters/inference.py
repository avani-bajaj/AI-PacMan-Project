...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        positionsForEachGhost = [((self.legalPositions * (self.numParticles // len(self.legalPositions))) + (random.sample(self.legalPositions, k=(self.numParticles % len(self.legalPositions))))) for _ in range(self.numGhosts)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in beliefs:
            for i in range(self.numGhosts):
                beliefs[particle] *= self.getObservationProb(observation[i], pacmanPosition, particle[i], jailPositions[i])
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                nextPosDistribution = self.getPositionDistribution(gameState, oldParticle, i, self.ghostAgents[i])
